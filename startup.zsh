...
#start minikube 
#mac only : open docker with line 6 to start the daemon  
# open -a Docker
...
kubectl get secret --namespace default grafana -o jsonpath="{.data.admin-password}" | base64 --decode > grafana/secret.txt
...
sleep 5  
curl -X POST -H "Content-Type: application/json" -d '{"name":"apikeycurl0", "role": "Admin"}' http://admin:`cat grafana/secret.txt`@localhost:3000/api/auth/keys > grafana/api_token.json
